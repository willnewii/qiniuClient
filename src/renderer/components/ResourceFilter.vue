...
<template>
  ...
  <Modal v-model="model_Query.show" title="请选择你要筛选的范围" @on-ok="filter">
    ...
    <Slider v-model="model_Query.fileSize" range :min="0" :max="model_Query.sizeArray.length - 1" show-tip="never"></Slider>
    <span>按文件日期：</span>
    {{ tipFormatDate(model_Query.fileDate[0]) }} ~ {{ tipFormatDate(model_Query.fileDate[1]) }}
    <Slider v-model="model_Query.fileDate" range :min="0" :max="model_Query.dateArray.length - 1" show-tip="never"></Slider>
  </Modal>
</template>

<script>
import { util, EventBus, Constants } from '../service/index'
import { resource } from '../mixins'
...
export default {
  ...
  methods: {
    ...
    filter() {
      ...
      let result = this.model_Query.sizeArray.filter(item => {
        return item.fsize >= sizeMin && item.fsize <= sizeMax && item.putTime >= dateMin && item.putTime <= dateMax
      })

      EventBus.$emit(Constants.Event.updateFiles, { source: result, flatten: true })
    },
    tipFormatSize(value) {
