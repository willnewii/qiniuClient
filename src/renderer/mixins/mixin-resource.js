...
export default {
  ...
  methods: {
    ...
    getResourceUrl(file) {
      ...
      return this.bucket.generateUrl(file.key, this.setup.deadline)
    },
    preview(file) {
    ...
    copyFileUrl(file, copyType) {
      let text = util.getClipboardText(copyType ? copyType : this.setup.copyType, this.getResourceUrl(file), file)
    ...
    resourceDownload(file) {
      ...
      if (this.baseDir) {
        ...
        if (this.setup.downloaddir) {
          option.directory = this.setup.downloaddir
        }
    ...
    resourceUpload(file) {
      ...
      this.bucket.createFile(param, /^https?:\/\//.test(file.path) ? Constants.UploadType.FETCH : Constants.UploadType.UPLOAD, callback)
    },
    ...
    resourceRemove(file, noAsk = false) {
      ...
      if (this.setup.deleteNoAsk || noAsk) {
