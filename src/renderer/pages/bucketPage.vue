...
<template>
  <div class="bucketpage" v-if="bucket">
    ...
    <div class="dir-layout">
      <div class="header-dir-view horizontal">
        <Breadcrumb separator=">">
          ...
          <template v-if="bucket.folderPath">
            <div class="bread-sub" v-for="(item, index) in bucket.folderPath.split('/')" :key="item" @click="changeFolderPath(index)">
      ...
      <div class="header-button-view">
        <template v-if="bucket.selection.length > 0">
          ...
          <Button v-if="!setup_hiddenDelBtn" class="btn" size="small" @click="askRemove()" icon="md-trash" type="error">删除({{ bucket.selection.length }})</Button>
        </template>
    ...
    <Modal v-model="modelDeleteAsk" title="确认删除文件？" class-name="vertical-center-modal" @on-ok="callRemove" @on-cancel="cleanSelection">
      <div class="file-list">
        <template>
          <p v-for="file in bucket.selection" :key="file.key">{{ file.key }}</p>
        </template>
      </div>
    </Modal>
  </div>
