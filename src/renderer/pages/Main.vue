<template>
  <div class="layout">
    ...
    <Spin size="large" fix v-if="loading.show">
      ...
      <Button @click="closeLoading" style="position: absolute; right: -50px; top: -50px" type="text" icon="ios-close-circle"></Button>
    </Spin>
    <!-- 分页加载提示框-->
    <Modal v-model="paging.show" class-name="vertical-center-modal" ok-text="开启" :closable="true" :mask-closable="false">
      <div class="paging-view">
        ...
        <p class="client-link" @click="openBrowser('https://github.com/willnewii/qiniuClient/wiki/%E6%95%B0%E6%8D%AE%E5%8A%A0%E8%BD%BD%E6%96%B9%E5%BC%8F')">数据加载方式区别</p>
      </div>
    </Modal>
    <!-- 文件拖拽提示框-->
...
export default {
  ...
  methods: {
    ...
    closeLoading() {
      ...
      if (this.loading && this.loading.cancel) {
        this.loading.cancel()
      }
    },
  },
