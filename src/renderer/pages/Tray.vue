            ...
            <div class="item" v-for="item of logs">
                <div v-if="item.code == 0">
                    ...
                    <div>
                        ...
                        <i-button class="btn" type="primary" size="small" @click="copyFileUrl(item.key)"
                            >复制路径</i-button
                        >
                        <i-button class="btn" type="primary" size="small" @click="openInFolder(item.path)"
                            >源文件
                        </i-button>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        uploadFile(file) {
            ...
            let key = ""
            key =
                (this.config.tray.bucket_dir ? this.config.tray.bucket_dir + "/" : "") +
                util.getFileNameWithFolder(file)
            let log = {
                path: file.path,
                key
            }
